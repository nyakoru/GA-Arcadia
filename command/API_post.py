...
class APIpost:
    """API endpoint\n 
    games | variants | sets | products | products_files | products_sets | directus_files"""
    endpoint: str
    ...
    def locate_endpoint(self, endpoint: str) -> str:
        API_ENDPOINT = {
            ...
            'products_translations':'/items/products_translations'
        }
        try:
    ...
    def post_files(self, files, json_data):
        data = self.files_access()
        response = data.posting_files(files=files, data=json_data)
        return response