...
def derivedUUID_(id_, string):
    return str(uuid.uuid5(id_, string))

def derivedUUID2_(parent1:uuid, parent2:str, salt:str="child"):
    if type(parent1) != uuid:
        parent1 = uuid.UUID(parent1)
    name = f"{parent2}{salt}"
    return str(uuid.uuid5(parent1, name))